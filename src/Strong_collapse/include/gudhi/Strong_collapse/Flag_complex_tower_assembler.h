...
public:
	Flag_complex_tower_assembler(const std::size_t num_vertices)
		...
		current_rename_counter_ = num_vertices;
	}
	...
	void build_tower_for_two_complexes(Flag_complex_sparse_matrix<SimplicialComplex>& mat_1,
									   ...
									   const Filtration_value filtration_value
									   //, const std::string& outFile = ""
									  )
	{
		std::cout << "fil: " << filtration_value << "\n";
		...
		//if (myfile.is_open() || outFile.empty()) {
			...
			for (auto& v : mat_1.vertex_set()) {
				...
				{
//					if (!outFile.empty() && mat_1.membership(collapsed_to->second)) {
//						myfile << filtration_value << " i " << renamed_vertices_.at(collapsed_to->second) << std::endl;
//						myfile << filtration_value << " c " << renamed_vertices_.at(v) << " " << renamed_vertices_.at(collapsed_to->second) << std::endl;
//					}

					flag_filtration_.active_strong_expansion(renamed_vertices_.at(v), renamed_vertices_.at(collapsed_to->second),
															 filtration_value);	//relable instead of collapse if renamed_vertices_.at(collapsed_to->second) not contained
					renamed_vertices_.at(v) = current_rename_counter_;
//					std::cout << "current_rename_counter_: " << current_rename_counter_ << "\n";
					...
					// then collapse
					std::cout << "collapsed_to->second: " << collapsed_to->second << "\n";
			...
			for (const Edge& e : mat_2.all_edges()) {
				...
				if (!mat_1.membership(u)) {
					std::cout << "v1fil: " << filtration_value << "\n";
...
//					}
				...
				if (!mat_1.membership(v)) {
					std::cout << "v2fil: " << filtration_value << "\n";
...
//					}
				...
				if (!mat_1.membership(e)) {
					std::cout << "efil: " << filtration_value << "\n";
