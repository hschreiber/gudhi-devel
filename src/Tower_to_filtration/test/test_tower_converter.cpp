...
bool test_output_stream_first_line(int *dim, int *filtrationValue, std::vector<vertex> *vertices){
	std::string line;
	if (getline(ss, line, '\n')){
...
BOOST_AUTO_TEST_CASE_TEMPLATE(insertion_test, ComplexType, complex_types)
{
	...
	BOOST_CHECK(!test_output_stream_first_line(&dim, &filtrationValue, &vertices));
