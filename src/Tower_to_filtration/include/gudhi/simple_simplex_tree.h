...
private:
	...
	Node* _insert_simplex_in_tree(simplex_vertex_range &simplex);
	Node* _insert_union(Node *simplex, vertex v);
	void _insert_node_in_dictionary(Node *simplex);
	void _delete_node_from_dictionary(Node *simplex);
	void _expand_node(Node *simplex, int maxDim, std::vector<simplex_handle>& addedSimplices);
	Node* _find(simplex_vertex_range &simplex);
	bool _is_coface(Node *node, Node *simplex);
	bool _are_all_facets_of_union_inserted(Node *facet, vertex endLabel);
	void _get_simplices_in_subtree(Node *node, std::vector<Node*> &simplices);
	vertex _get_smallest_star(vertex v, vertex u, std::queue<Node*> &qv, std::queue<Node*> &qu);
	bool _get_smallest_star_find_next_node(vertex v, vertex toAvoid, std::vector<label_dictionary>::size_type &currentHeight,
	...
	bool _get_smallest_star_find_next_root(vertex v, vertex toAvoid, std::vector<label_dictionary>::size_type &currentHeight,
										   Node *&currentRoot, Node *&startRoot, Node *&currentNode, bool &currentNodeIsRoot);
	Node* _get_opposite_facet(Node *simplex, vertex v);
	void _print_from_node(Node *node, std::string prefix);
};
...
inline Simplex_tree::~Simplex_tree()
{
	for (std::vector<label_dictionary>::size_type i = 0; i < dictionaries_.size(); i++){
		for (label_dictionary::iterator it = dictionaries_.at(i).begin(); it != dictionaries_.at(i).end(); it++)
			delete it->second;
	}
}

inline std::vector<Simplex_tree::simplex_handle> Simplex_tree::insert_simplex(simplex_vertex_range &simplex)
{
	Node *simplexNode = _insert_simplex_in_tree(simplex);
	if (simplexNode == nullptr)
		return std::vector<Simplex_tree::simplex_handle>();
	...
	return std::vector<Simplex_tree::simplex_handle>(1, simplexNode);
}

inline std::vector<Simplex_tree::simplex_handle> Simplex_tree::insert_simplex_and_faces(simplex_vertex_range &simplex)
{
	int dim = simplex.size() == 0 ? 0 : simplex.size() - 1;
	std::vector<Node*> nodes;
	std::vector<Node*> tmpNodes;
	Node *vertexNode;
	std::vector<Simplex_tree::simplex_handle> addedSimplices;

	if (_find(simplex) != nullptr) return addedSimplices;

	if (static_cast<int>(dictionaries_.size()) <= dim + 1)
		dictionaries_.resize(dim + 2);
//	if (numberOfSimplices_ == 0) dictionaries_.push_back(label_dictionary());
	...
	for (simplex_vertex_range::size_type i = 0; i < simplex.size(); ++i){
		...
		if (vertices.find(v) != vertices.end()){
			...
			++numberOfSimplices_;
			++maxIndex_;
			addedSimplices.push_back(vertexNode);
		}
...
//				if (static_cast<int>(dictionaries_.size()) == dim + 1)
//					dictionaries_.push_back(label_dictionary());
				Node *vNode = new Node(v, maxIndex_ + 1, dim + 1, node);
				children.emplace(v, vNode);
				_insert_node_in_dictionary(vNode);
				++numberOfSimplices_;
				++maxIndex_;
				addedSimplices.push_back(vNode);
			}
...
inline std::vector<Simplex_tree::simplex_handle> Simplex_tree::insert_edge_and_expand(vertex u, vertex v, int maxDim)
{
	simplex_vertex_range vect;
	std::vector<simplex_handle> addedSimplices;

	vect.push_back(u);
	std::vector<Node*> nodes = insert_simplex(vect);
	if (!nodes.empty()) addedSimplices.push_back(nodes.front());

	vect.at(0) = v;
	nodes = insert_simplex(vect);
	if (!nodes.empty()) addedSimplices.push_back(nodes.front());
	...
	nodes = insert_simplex(vect);
	if (!nodes.empty()) {
		addedSimplices.push_back(nodes.front());
		if (maxDim > 1 || maxDim == -1) _expand_node(nodes.front(), maxDim, addedSimplices);
	}

	return addedSimplices;
}

inline std::vector<Simplex_tree::index> Simplex_tree::remove_simplex(simplex_vertex_range &simplex)
{
	Node *simplexNode = _find(simplex);
	return remove_simplex(simplexNode);
}

inline std::vector<Simplex_tree::index> Simplex_tree::remove_simplex(simplex_handle simplex)
{
	std::vector<index> removedIndices;
	std::vector<Node*> cofaces = get_cofaces(simplex);
	...
	for (std::vector<Node*>::size_type i = 0; i < cofaces.size(); i++){
		...
		removedIndices.push_back(toDelete->get_insertion_index());

		if (toDelete->get_dim() > 0) toDelete->get_parent()->get_children().erase(toDelete->get_label());
		_delete_node_from_dictionary(toDelete);
	...
	if (_get_smallest_star(v, u, qv, qu) == v){
		while (!qv.empty()){
			...
			if (s->get_parent() != nullptr){
				closedStar.push_back(_get_opposite_facet(s, v));
			}
		...
		while (!qu.empty()){
			...
			qu.pop();
			if (s->get_parent() != nullptr){
				closedStar.push_back(_get_opposite_facet(s, u));
			}
			closedStar.push_back(s);
		}
...
inline std::vector<Simplex_tree::simplex_handle> Simplex_tree::get_boundary(simplex_handle simplexNode)
{
	std::vector<simplex_handle> boundary;

	if (simplexNode->get_parent() == nullptr) return boundary;
...
inline Simplex_tree::index Simplex_tree::get_key(Simplex_tree::simplex_handle simplex)
{
	return simplex->get_insertion_index();
}
...
inline std::vector<Simplex_tree::simplex_handle> Simplex_tree::get_cofaces(simplex_handle simplex)
{
	...
	for (int d = dictionaries_.size() - 1; d >= simplex->get_dim(); d--){
		if (dictionaries_.at(d).find(simplex->get_label()) != dictionaries_.at(d).end()){
			...
			if (_is_coface(it, simplex)) _get_simplices_in_subtree(it, cofaces);
			it = it->get_next();
...
inline std::vector<Simplex_tree::simplex_handle> Simplex_tree::get_cofacets(simplex_handle simplex)
{
	std::vector<Node*> cofacets;

	if (static_cast<int>(dictionaries_.size()) > simplex->get_dim() &&
		...
		Node *startingNode = it;
		if (_is_coface(it, simplex)) cofacets.push_back(it);
		it = it->get_next();
		while (it != startingNode){
			if (_is_coface(it, simplex)) cofacets.push_back(it);
			it = it->get_next();
...
inline void Simplex_tree::print()
{
	...
	for (auto it = dictionaries_.front().begin(); it != dictionaries_.front().end(); ++it){
		vertex = it->second;
		_print_from_node(vertex, "");
	}
}

inline Simplex_tree::Node* Simplex_tree::_insert_simplex_in_tree(simplex_vertex_range &simplex)
{
	int dim = simplex.size() - 1;

	if (dim == 0){
		if (numberOfSimplices_ == 0)
			dictionaries_.push_back(label_dictionary());
		label_dictionary &vertices = dictionaries_.front();
		if (vertices.find(simplex.at(0)) != vertices.end())
			return nullptr;
	...
	if (it->get_children().find(simplex.back()) != it->get_children().end())
		return nullptr;

	if (static_cast<int>(dictionaries_.size()) == dim)
		dictionaries_.push_back(label_dictionary());
	Node *simplexNode = new Node(simplex.back(), maxIndex_ + 1, dim, it);
	it->get_children().emplace(simplex.back(), simplexNode);
	_insert_node_in_dictionary(simplexNode);

	return simplexNode;
}

inline Simplex_tree::Node* Simplex_tree::_insert_union(Node *simplex, vertex v)
{
	Node *it = simplex;
	...
	if (!_are_all_facets_of_union_inserted(it, endLabel))
		return nullptr;

	if (static_cast<int>(dictionaries_.size()) == simplex->get_dim() + 1)
		dictionaries_.push_back(label_dictionary());
...
inline void Simplex_tree::_insert_node_in_dictionary(Node *simplex)
{
...
inline void Simplex_tree::_delete_node_from_dictionary(Node *simplex)
{
	label_dictionary &dict = dictionaries_.at(simplex->get_dim());
...
inline void Simplex_tree::_expand_node(Node *simplex, int maxDim, std::vector<simplex_handle> &addedSimplices)
{
	std::vector<Node*> cofacets = get_cofacets(simplex->get_parent());

	for (Node *cofacet : cofacets){
		if (cofacet == simplex) continue;
		Node *node = _insert_union(cofacet, simplex->get_label());
		if (node != nullptr){	//if node could be inserted, i.e. all its facets were there and it-self was not already inserted
			addedSimplices.push_back(node);
			if (node->get_dim() < maxDim || maxDim == -1) _expand_node(node, maxDim, addedSimplices);
		}
	}
}

inline Simplex_tree::Node *Simplex_tree::_find(simplex_vertex_range &simplex)
{
...
inline bool Simplex_tree::_is_coface(Node *node, Node *simplex)
{
	Node *nodeIt = node;
...
inline bool Simplex_tree::_are_all_facets_of_union_inserted(Node *facet, vertex endLabel)
{
	if (facet->get_parent() == nullptr)
		return dictionaries_.front().find(endLabel) != dictionaries_.front().end();
...
inline void Simplex_tree::_get_simplices_in_subtree(Node *node, std::vector<Node *> &simplices)
{
	simplices.push_back(node);
	for (auto it = node->get_children().begin(); it != node->get_children().end(); it++){
		_get_simplices_in_subtree(it->second, simplices);
	}
}

inline Simplex_tree::vertex Simplex_tree::_get_smallest_star(vertex v, vertex u, std::queue<Node*> &qv, std::queue<Node*> &qu)
{
	...
	while (continueV && continueU){
		...
		continueV = _get_smallest_star_find_next_node(v, u, currentHeightV, currentRootV, startRootV, currentNodeV, currentNodeIsRootV, childrenV, vit, tv);
		continueU = _get_smallest_star_find_next_node(u, v, currentHeightU, currentRootU, startRootU, currentNodeU, currentNodeIsRootU, childrenU, uit, tu);
	}
...
inline bool Simplex_tree::_get_smallest_star_find_next_node(vertex v, vertex toAvoid, std::vector<label_dictionary>::size_type &currentHeight, Node *&currentRoot,
														   ...
														   label_dictionary &children, label_dictionary::iterator &it, std::queue<Node*> &tail)
{
	...
	while (it == children.end()){
		if (!tail.empty()){
			...
			return _get_smallest_star_find_next_root(v, toAvoid, currentHeight, currentRoot, startRoot, currentNode, currentNodeIsRoot);
		}
	}

	if (it->first == toAvoid){
		return _get_smallest_star_find_next_node(v, toAvoid, currentHeight, currentRoot, startRoot, currentNode, currentNodeIsRoot, children, it, tail);
	}
...
inline bool Simplex_tree::_get_smallest_star_find_next_root(vertex v, vertex toAvoid, std::vector<label_dictionary>::size_type &currentHeight, Node *&currentRoot,
														   Node *&startRoot, Node *&currentNode, bool &currentNodeIsRoot)
{
	if (currentRoot->get_next() == startRoot){
		currentHeight++;
		while (currentHeight < dictionaries_.size() && dictionaries_.at(currentHeight).find(v) == dictionaries_.at(currentHeight).end())
			currentHeight++;

		if (currentHeight == dictionaries_.size())
			return false;
	...
	if (_is_coface(currentRoot, dictionaries_.at(0).at(toAvoid)))
		return _get_smallest_star_find_next_root(v, toAvoid, currentHeight, currentRoot, startRoot, currentNode, currentNodeIsRoot);
...
inline Simplex_tree::Node *Simplex_tree::_get_opposite_facet(Node *simplex, vertex v)
{
...
inline void Simplex_tree::_print_from_node(Node *node, std::string prefix)
{
	...
	for (auto it = node->get_children().begin(); it != node->get_children().end(); ++it){
		_print_from_node(it->second, prefix);
	}
}
...
inline Simplex_tree::label_dictionary& Simplex_tree::Node::get_children()
{
	return children_;
}
